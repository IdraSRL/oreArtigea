...
try {
    try { app = initializeApp(firebaseConfig, 'gradimento'); } catch(e) { app = getApp('gradimento'); }
    db = getFirestore(app);
    window.__GRADIMENTO_INITED__ = true;
} catch (error) {
    console.warn('Errore inizializzazione Firebase Gradimento:', error);
}
