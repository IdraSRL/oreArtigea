...
class AdminValutazioneManager {
    ...
    async handleAddProduct() {
        ...
        console.log('🔍 [DEBUG] Inizio handleAddProduct');
        console.log('🔍 [DEBUG] editingProduct:', this.editingProduct);
        ...
        console.log('🔍 [DEBUG] imageFile:', imageFile ? imageFile.name : 'nessun file');
        ...
        console.log('🔍 [DEBUG] productData creato:', productData);
        console.log('🔍 [DEBUG] Controllo campi:');
        console.log('  - id:', productData.id, 'valido:', !!productData.id);
        console.log('  - name:', productData.name, 'valido:', !!productData.name);
        console.log('  - tagMarca:', productData.tagMarca, 'valido:', !!productData.tagMarca);
        console.log('  - tagTipo:', productData.tagTipo, 'valido:', !!productData.tagTipo);
        
        // Se è un nuovo prodotto, aggiungi createdAt
        if (!this.editingProduct) {
            productData.createdAt = serverTimestamp();
            console.log('🔍 [DEBUG] Aggiunto createdAt per nuovo prodotto');
        }
        ...
        if (!productData.id || !productData.name || !productData.tagMarca || !productData.tagTipo) {
            ...
            console.log('  - productData.tagMarca:', productData.tagMarca);
            console.log('  - productData.tagTipo:', productData.tagTipo);
            // Riabilita il pulsante in caso di errore
            saveBtn.disabled = false;
            saveBtn.innerHTML = originalText;
        ...
        if (!this.editingProduct && this.products.find(p => p.id === productData.id)) {
            console.log('❌ [DEBUG] ID già esistente per nuovo prodotto');
            // Riabilita il pulsante in caso di errore
            saveBtn.disabled = false;
            saveBtn.innerHTML = originalText;
        ...
        console.log('🔍 [DEBUG] Controllo ID duplicato superato');

        try {
            console.log('🚀 [DEBUG] Tentativo di salvataggio su Firestore...');
            await setDoc(doc(db, 'Products', productData.id), productData);
            
            const action = this.editingProduct ? 'aggiornato' : 'aggiunto';
            console.log('✅ [DEBUG] Salvataggio completato:', action);
    ...
    async uploadProductImage(file, productId) {
        console.log('🔄 Iniziando upload immagine per prodotto:', productId);
        ...
        try {
            console.log('🚀 Inviando richiesta POST...');
            ...
            console.log('📄 Testo risposta grezzo:', responseText);
            ...
            if (responseText.includes('<br />') || responseText.includes('<?php') || responseText.includes('<html>')) {
                console.error('❌ Risposta contiene HTML di errore PHP:', responseText.substring(0, 200));
                return {
                    success: false,
            ...
            try {
                ...
                console.error('❌ Contenuto risposta:', responseText.substring(0, 500));
                return {
                    success: false,
